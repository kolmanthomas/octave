...
%% Test scalar transpose - should return scalar unchanged
%!test
%! obj = foo_value_class (4, 48, 50000);
...
%% Test scalar ctranspose - should work same as transpose for classdef
%!test
...
%! assert (isequal (result(1, 1).rate, arr(1, 1).rate));
%! assert (isequal (result(2, 1).rate, arr(1, 2).rate));
%! assert (isequal (result(3, 1).rate, arr(1, 3).rate));
...
%! assert (isequal (result(1).rate, obj1.rate));
%! assert (isequal (result(2).rate, obj2.rate));
%! assert (isequal (result(3).rate, obj3.rate));
...
%! assert (isequal (result(2,2).rate, arr(2, 2).rate));
%! assert (isequal (result(3,1).rate, arr(1, 3).rate));
%! assert (isequal (result(3,2).rate, arr(2, 3).rate));

%% Test ctranspose on arrays - should work same as transpose
%!test
%! obj1 = foo_value_class (4, 48, 50000);
%! obj2 = foo_value_class (3, 36, 25000);
%! arr = [obj1, obj2];
...
%! assert (isequal (result1(1).rate, result2(1).rate));
%! assert (isequal (result1(2).rate, result2(2).rate));
...
%! assert (isequal (result(1,1).rate, arr(1,1).rate));
%! assert (isequal (result(1,2).rate, arr(1,2).rate));
%! assert (isequal (result(2,1).rate, arr(2,1).rate));
%! assert (isequal (result(2,2).rate, arr(2,2).rate));

%% Test that transpose preserves object identity for scalars
%!test
%! obj = foo_value_class (4, 48, 50000);
%! result = transpose (obj);
%! result.rate = 5;
...
%! assert (isequal (obj.rate, 4));
%! assert (isequal (obj.term, 48));
%! assert (isequal (obj.principle, 50000));
%! assert (isequal (result.rate, 5));
%! assert (isequal (result.term, 60));
%! assert (isequal (result.principle, 75000));
